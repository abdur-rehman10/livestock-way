...
export interface ResourcesApplicationThread {
  id: number;
  listing_id: number;
...
export interface ResourcesApplicationMessage {
  id: number;
  thread_id: number;
...
function mapThreadRow(row: any): ResourcesApplicationThread {
  return {
    id: Number(row.id),
...
function mapMessageRow(row: any): ResourcesApplicationMessage {
  return {
    id: Number(row.id),
...
export async function getUserResourcesThreads(userId: number): Promise<ResourcesApplicationThread[]> {
  const result = await pool.query(
    ...
    SELECT 
      ...
      (
        SELECT m.message 
        FROM resources_application_messages m 
    ...
    FROM resources_application_threads t
    INNER JOIN resources_listings l ON l.id = t.listing_id
    INNER JOIN resources_applications app ON app.id = t.application_id
...
export async function getResourcesThreadById(threadId: number, userId: number): Promise<ResourcesApplicationThread | null> {
  const result = await pool.query(
    `
    SELECT 
      t.*,
...
export async function getResourcesThreadByListingAndApplication(
  listingId: number,
  applicationId: number,
  userId: number
): Promise<ResourcesApplicationThread | null> {
  const result = await pool.query(
    `
    SELECT 
      t.*,
...
export async function getResourcesThreadMessages(threadId: number, userId: number): Promise<ResourcesApplicationMessage[]> {
  // First verify user has access to this thread and update last_read_at
  const threadCheck = await pool.query(
    "SELECT listing_poster_user_id, applicant_user_id FROM resources_application_threads WHERE id = $1 AND (listing_poster_user_id = $2 OR applicant_user_id = $2) AND is_active = TRUE",
    [threadId, userId]
  );
  ...
  if (isListingPoster) {
    await pool.query(
      "UPDATE resources_application_threads SET listing_poster_last_read_at = NOW() WHERE id = $1",
      [threadId]
    );
  } else {
    await pool.query(
      "UPDATE resources_application_threads SET applicant_last_read_at = NOW() WHERE id = $1",
      [threadId]
    );
  }
...
export async function sendResourcesThreadMessage(
  threadId: number,
  senderUserId: number,
  senderRole: string,
  message: string,
  attachments?: any[]
): Promise<ResourcesApplicationMessage> {
  ...
  const thread = await pool.query(
    ...
    SELECT 
      ...
      applicant_user_id,
      first_message_sent
    FROM resources_application_threads
  ...
  const result = await pool.query(
    `
    INSERT INTO resources_application_messages (
      thread_id,
      sender_user_id,
      sender_role,
      message,
