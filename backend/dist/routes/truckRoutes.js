...
router.post("/", (0, rbac_1.requireRoles)(["hauler"], { allowSuperAdminOverride: false }), (0, auditLogger_1.auditRequest)("truck:create"), async (req, res) => {
    try {
        const { truck_name, plate_number, capacity_lbs, capacity, equipment_type, truck_type, species_supported, notes, description, height_m, width_m, length_m, axle_count, max_gross_weight_kg, max_axle_weight_kg, hazmat_permitted, } = req.body;
        ...
        const maxGrossValue = max_gross_weight_kg !== undefined && max_gross_weight_kg !== null
        ...
        const maxAxleValue = max_axle_weight_kg !== undefined && max_axle_weight_kg !== null
        ...
        const existing = await database_1.pool.query(`SELECT * FROM trucks WHERE hauler_id = $1 AND plate_number = $2 LIMIT 1`, [haulerId, plate_number]);
        if (existing.rowCount && existing.rows[0]?.status === "inactive") {
            const reactivate = await database_1.pool.query(`
          UPDATE trucks
          SET truck_type = $2,
...
router.delete("/:id", (0, rbac_1.requireRoles)(["hauler"], { allowSuperAdminOverride: false }), (0, auditLogger_1.auditRequest)("truck:delete", (req) => `truck:${req.params.id}`), async (req, res) => {
    try {
        ...
        const tripBlock = await database_1.pool.query(`
        SELECT 1
        FROM trips
        WHERE truck_id = $1
      ...
      `, [id, activeTripStatuses]);
        ...
        const availabilityBlock = await database_1.pool.query(`
        SELECT 1
...
router.patch("/:id", (0, rbac_1.requireRoles)(["hauler"], { allowSuperAdminOverride: false }), (0, auditLogger_1.auditRequest)("truck:update", (req) => `truck:${req.params.id}`), async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (Number.isNaN(id)) {
            return res.status(400).json({ message: "Invalid truck id" });
        ...
        const haulerId = await (0, profileHelpers_1.ensureHaulerProfile)(Number(req.user?.id));
        const { plate_number, truck_type, capacity_weight_kg, height_m, width_m, length_m, axle_count, max_gross_weight_kg, max_axle_weight_kg, hazmat_permitted, truck_name, species_supported, notes, } = req.body ?? {};
        const parsed = {
