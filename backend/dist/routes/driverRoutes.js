...
router.post("/", async (req, res) => {
    try {
        const { hauler_id, first_name, last_name, phone, email, license_number, } = req.body;
        if (!hauler_id || !first_name || !last_name || !phone || !license_number) {
            return res.status(400).json({
                message: "hauler_id, first_name, last_name, phone and license_number are required",
            });
        }
        const haulerCheck = await database_1.pool.query("SELECT id FROM haulers WHERE id = $1 LIMIT 1", [hauler_id]);
