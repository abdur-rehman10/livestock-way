...
async function runStatsQuery(sql, fallback) {
    try {
...
router.use(auth_1.default, (0, rbac_1.requireRoles)(["super-admin"], { allowSuperAdminOverride: false }));
router.get("/stats", async (_req, res) => {
    try {
        const [usersStats, loadStats, tripStats, paymentStats, kycStats, disputeStats, supportStats,] = await Promise.all([
            runStatsQuery(`
          SELECT
...
router.get("/earnings", async (_req, res) => {
    try {
        const earningsStatsResult = await database_1.pool.query(`
      SELECT
      ...
      FROM payments
    `);
        const earningsItemsResult = await database_1.pool.query(`
        SELECT
          p.id,
          p.trip_id,
          p.load_id,
...
router.put("/pricing/hauler-individual", async (req, res) => {
    try {
        const { monthly_price } = (0, pricing_1.validateIndividualPricingInput)(req.user?.user_type, req.body?.monthly_price);
        const existing = await database_1.pool.query(`SELECT id FROM pricing_configs WHERE target_user_type = 'HAULER_INDIVIDUAL' AND is_active = TRUE LIMIT 1`);
        if (existing.rowCount) {
            const updated = await database_1.pool.query(`
        ...
        await database_1.pool.query(`UPDATE pricing_configs SET is_active = FALSE WHERE target_user_type = 'HAULER_INDIVIDUAL'`);
        const inserted = await database_1.pool.query(`
      ...
      `, [monthly_price]);
        ...
        await database_1.pool.query("ROLLBACK").catch(() => null);
        const message = error?.message || "Failed to update individual pricing";
        const status = message.includes("super admins") || message.includes("positive number")
            ? 400
            : 500;
        if (status === 500)
            console.error("admin pricing individual put error", error);
        return res.status(status).json({ message });
    }
});
router.get("/pricing/hauler-company", async (_req, res) => {
    try {
        const configResult = await database_1.pool.query(`
        SELECT *
        FROM pricing_configs
...
router.put("/pricing/hauler-company", async (req, res) => {
    try {
        const validatedTiers = (0, pricing_1.validateCompanyPricingInput)(req.user?.user_type, req.body?.tiers || []);
        await database_1.pool.query("BEGIN");
        let config = await database_1.pool.query(`
        SELECT *
        FROM pricing_configs
        WHERE target_user_type = 'HAULER_COMPANY' AND is_active = TRUE
        ORDER BY created_at DESC
      ...
      `);
        if (!config.rowCount) {
            config = await database_1.pool.query(`
        ...
        await database_1.pool.query(`DELETE FROM pricing_company_tiers WHERE pricing_config_id = $1`, [configId]);
        if (validatedTiers.length > 0) {
            const values = [];
            const placeholders = [];
            validatedTiers.forEach((tier, idx) => {
                values.push(configId, tier.name, tier.min_vehicles, tier.max_vehicles, tier.monthly_price, tier.sales_form_link, tier.sort_order ?? idx, tier.is_enterprise);
                const base = idx * 8;
        ...
        await database_1.pool.query(`UPDATE pricing_configs SET updated_at = NOW(), is_active = TRUE WHERE id = $1`, [configId]);
        const tiers = (await database_1.pool.query(`
          SELECT *
          FROM pricing_company_tiers
          WHERE pricing_config_id = $1
          ORDER BY sort_order ASC, id ASC
        ...
        const status = message.includes("super admins") ||
            ...
            : 500;
        if (status === 500)
            console.error("admin pricing company put error", error);
        return res.status(status).json({ message });
    }
});
...
router.patch("/users/:id/status", (0, auditLogger_1.auditRequest)("admin:user-status", (req) => `user:${req.params.id}`), async (req, res) => {
    try {
...
router.patch("/support-tickets/:id/status", (0, auditLogger_1.auditRequest)("admin:support-update", (req) => `support_ticket:${req.params.id}`), async (req, res) => {
    try {
...
router.get("/support-tickets/:ticketId/messages", async (req, res) => {
    try {
        ...
        const messages = await database_1.pool.query(`
        SELECT id,
               ticket_id,
               sender_user_id,
               sender_role,
...
router.post("/support-tickets/:ticketId/messages", (0, auditLogger_1.auditRequest)("admin:support-message", (req) => `support_ticket:${req.params.ticketId}`), async (req, res) => {
    try {
        const ticketId = Number(req.params.ticketId);
        if (Number.isNaN(ticketId)) {
            return res.status(400).json({ message: "Invalid ticket id" });
